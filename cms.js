...
async function requireAuth(){
  ...
  try{currentUser=await api("/auth/v1/user")}
  catch{
    if(!(await renew())){clearSession();location.replace(ADMIN_BASE+"/login");return false}
    currentUser=await api("/auth/v1/user")
  }

  const memberships=await api(`/rest/v1/site_members?user_id=eq.${currentUser.id}&select=site_id,role`);
  if(!memberships.length){
    clearSession();location.replace(ADMIN_BASE+"/login?error=unauthorized");return false
  }
  const ids=memberships.map(x=>x.site_id);
  const sites=await api(`/rest/v1/sites?id=in.(${ids.join(",")})&select=id,name,domain,timezone,locale`);
  currentSites=sites.map(s=>{
...
function adminTools(){
  const a=$(".admin-actions");if(!a||$("#rvf-logout"))return;
  const role=document.createElement("span");role.className="subtle";role.id="rvf-current-role";role.textContent=currentUser.cmsRole;
  const b=document.createElement("button");b.id="rvf-logout";b.className="btn";b.type="button";b.textContent="Logout";
  b.onclick=async()=>{try{await api("/auth/v1/logout",{method:"POST"})}catch{}clearSession();location.replace(ADMIN_BASE+"/login")};
  a.prepend(role,b)
}
...
function renderAdminNavigation(){
  ...
  side.innerHTML=`
    ...
    <div class="admin-group-label">Content</div>
    <nav class="admin-menu">
      ${item(ADMIN_BASE+"/articles","Articles","▤")}
      ${item(ADMIN_BASE+"/calendar","Calendar","◫")}
      ${item(ADMIN_BASE+"/media","Media","▧")}
    </nav>
    <div class="admin-group-label">Growth</div>
    <nav class="admin-menu">
      ${item(ADMIN_BASE+"/analytics","Performance","↗")}
      ${item(ADMIN_BASE+"/opportunities","Opportunities","◎")}
      ${item(ADMIN_BASE+"/internal-links","Internal Links","⛓")}
    </nav>
    <div class="admin-group-label">Site</div>
    <nav class="admin-menu">
      ${item(ADMIN_BASE+"/seo","SEO & Indexing","◉")}
      ${item(ADMIN_BASE+"/site-health","Site Health","✓")}
      ${item(ADMIN_BASE+"/redirects","Redirects","↪")}
    </nav>
    <div class="admin-group-label">System</div>
    <nav class="admin-menu">
      ${item(ADMIN_BASE+"/ai-jobs","Activity","⌁")}
      ${item(ADMIN_BASE+"/integrations","Integrations","⚙")}
    </nav>
    <div class="admin-side-foot"><span>Cloudflare + Supabase</span><small>GitHub command bridge active</small></div>
  `;
...
authReady.then(ok=>{if(ok)renderAdminNavigation()});
...
if(loginPath){
  ...
  $("#login")?.addEventListener("submit",async e=>{
    ...
    try{
      const s=await api("/auth/v1/token?grant_type=password",{method:"POST",auth:false,body:JSON.stringify({email:$("#email").value,password:$("#password").value})});
      saveSession(s);location.replace(ADMIN_BASE)
    }catch{$("#message").textContent="Sign in failed. Check your email and password."}
  })
}
async function site(){
...
async function loadArticles(){
  ...
  setupArticleBulkActions();
}
function renderArticles(rows){
  const body=$("#live-articles");if(!body)return;
  body.innerHTML=rows.map(a=>`<tr>
    <td class="select-col"><input type="checkbox" data-article-select value="${a.id}" aria-label="${esc(a.title)} seç"></td>
    <td><a href="${ADMIN_BASE}/quick-entry?id=${a.id}"><b>${esc(a.title)}</b></a><br><span class="subtle">/blog/${esc(a.slug)}</span></td>
    ...
    <button class="btn" data-archive="${a.id}">Archive</button><button class="btn danger" data-delete="${a.id}" data-delete-title="${esc(a.title)}">Delete</button></div></td>
    <td>${new Date(a.updated_at).toLocaleDateString()}</td></tr>`).join("")||'<tr><td colspan="7">Henüz içerik yok.</td></tr>';
  body.onclick=articleAction;
}
function setupArticleBulkActions(){
  const body=$("#live-articles"),bar=$("#bulk-toolbar"),count=$("#bulk-selected-count"),all=$("#select-all-articles");
  if(!body||!bar)return;
  const selected=()=>[...document.querySelectorAll("[data-article-select]:checked")].map(x=>x.value);
  const refresh=()=>{
    const ids=selected(),boxes=[...document.querySelectorAll("[data-article-select]")];
  ...
  body.onchange=e=>{if(e.target.matches("[data-article-select]"))refresh()};
  if(all)all.onchange=e=>{document.querySelectorAll("[data-article-select]").forEach(cb=>cb.checked=e.target.checked);refresh()};
  $("#bulk-clear")?.addEventListener("click",()=>{document.querySelectorAll("[data-article-select]").forEach(cb=>cb.checked=false);refresh()},{once:true});
  $("#bulk-archive")?.addEventListener("click",async e=>{
    const ids=selected();if(!ids.length||!confirm(`${ids.length} içerik arşive alınsın mı?`))return;
    e.currentTarget.disabled=true;
    try{await api(`/rest/v1/articles?id=in.(${ids.join(",")})`,{method:"PATCH",body:JSON.stringify({status:"archived",updated_by:currentUser.id})});await loadArticles()}
    catch(err){alert(`Toplu arşivleme başarısız: ${err.message}`)}
  },{once:true});
  $("#bulk-delete")?.addEventListener("click",async e=>{
    const ids=selected();if(!ids.length)return;
    if(!confirm(`${ids.length} içerik KALICI olarak silinecek. Bu işlem geri alınamaz. Devam edilsin mi?`))return;
    e.currentTarget.disabled=true;
    try{await api(`/rest/v1/articles?id=in.(${ids.join(",")})`,{method:"DELETE"});await loadArticles()}
    catch(err){alert(`Toplu silme başarısız: ${err.message}`)}
  },{once:true});
  refresh();
}
async function articleAction(e){
  const pub=e.target.closest("[data-publish]"),unpub=e.target.closest("[data-unpublish]"),archive=e.target.closest("[data-archive]"),del=e.target.closest("[data-delete]");
  try{
    if(pub){
      pub.disabled=true;
      ...
      if(!gate?.can_publish){
        ...
        throw new Error(`Publish gate failed${failed.length?": "+failed.join(", "):""}.`);
      }
    ...
    if(del){
      const title=del.dataset.deleteTitle||"Bu içerik";
      if(!confirm(`"${title}" kalıcı olarak silinsin mi?\n\nBu işlem geri alınamaz.`))return;
      del.disabled=true;
      await api(`/rest/v1/articles?id=eq.${del.dataset.delete}`,{method:"DELETE"});
      return loadArticles();
    }
...
window.RVFAdmin={adminJson,site,esc,formatSiteDateTime,token,ready:authReady};
...
async function initEditor(){
  ...
  form.addEventListener("input",()=>{
    dirty=true;
    if($("#autosave-state"))$("#autosave-state").textContent="Unsaved";
    updateUI();
  });
  ...
  updateUI();
  dirty=false;
  if($("#autosave-state"))$("#autosave-state").textContent=id?"Saved":"Save draft first";

  // -------- Revision history --------
  const revisionModal=$("#revision-modal");
  async function loadRevisions(){
    const list=$("#revision-list");
    if(!id){list.innerHTML='<div class="empty-state">Save the draft once to start version history.</div>';return}
    list.innerHTML='<div class="empty-state">Loading versions…</div>';
    try{
      const rows=await api(`/rest/v1/article_revisions?article_id=eq.${encodeURIComponent(id)}&select=id,revision_no,title,change_note,created_at&order=revision_no.desc`);
      list.innerHTML=rows.length?rows.map(r=>`<div class="revision-row"><div><b>Version ${r.revision_no}</b><span>${esc(r.change_note||"Manual snapshot")} · ${formatSiteDateTime(r.created_at)}</span></div><button class="btn" type="button" data-restore-revision="${esc(r.id)}">Restore to editor</button></div>`).join(""):'<div class="empty-state">No versions yet. A snapshot is created before major manual saves and publishing.</div>';
    }catch(err){list.innerHTML=`<div class="cms-message error">${esc(err.message)}</div>`}
  }
  $("#open-revisions")?.addEventListener("click",async()=>{revisionModal.hidden=false;await loadRevisions()});
  revisionModal?.querySelectorAll("[data-close-revisions]").forEach(el=>el.addEventListener("click",()=>revisionModal.hidden=true));
  $("#revision-list")?.addEventListener("click",async e=>{
    const btn=e.target.closest("[data-restore-revision]");if(!btn)return;
    try{
      const rows=await api(`/rest/v1/article_revisions?id=eq.${encodeURIComponent(btn.dataset.restoreRevision)}&select=title,content_markdown,seo_title,meta_description&limit=1`);
      const r=rows?.[0];if(!r)throw new Error("Revision not found.");
      form.title.value=r.title||form.title.value;
      form.content_markdown.value=r.content_markdown||"";
      form.seo_title.value=r.seo_title||"";
      form.meta_description.value=r.meta_description||"";
      dirty=true;
      if($("#autosave-state"))$("#autosave-state").textContent="Restored · unsaved";
      updateUI();revisionModal.hidden=true;
      message.textContent="Version restored in the editor. Save when you are ready.";
      message.className="cms-message success";
  ...
  $("#generate-featured-image")?.addEventListener("click",async e=>{
    ...
    try{
      const d=await adminJson("/api/media/generate",{
        method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({article_id:id,prompt,alt_text:alt,placement:"featured",steps:4})
      });
      featuredUrl.value=d.media?.public_url||d.article?.featured_image_url||"";
      form.featured_image_alt.value=d.media?.alt_text||alt;
      form.featured_image_prompt.value=prompt;
      syncFeaturedPreview();
      dirty=false;
      featuredStatus.textContent="AI featured image generated and attached.";
      featuredStatus.className="cms-message success";
    }catch(err){featuredStatus.textContent=err.message;featuredStatus.className="cms-message error"}
    finally{btn.disabled=false}
  });

  $("#generate-article-image")?.addEventListener("click",async e=>{
    const btn=e.currentTarget;
    if(!id){articleImageStatus.textContent="Save the draft once before generating an inline image.";articleImageStatus.className="cms-message error";return}
    const prompt=$("#article_image_prompt").value.trim();
    const alt=$("#article_image_alt").value.trim()||form.title.value.trim();
    const caption=$("#article_image_caption").value.trim();
    const after_heading=$("#article_image_heading").value.trim();
    if(prompt.length<10){articleImageStatus.textContent="Add a more descriptive AI image brief.";articleImageStatus.className="cms-message error";return}
    btn.disabled=true;articleImageStatus.textContent="Generating and inserting image…";articleImageStatus.className="cms-message";
    try{
      const d=await adminJson("/api/media/generate",{
        method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({article_id:id,prompt,alt_text:alt,caption,placement:"inline",after_heading,steps:4})
      });
      if(d.article?.content_markdown)form.content_markdown.value=d.article.content_markdown;
      dirty=false;updateUI();
      articleImageStatus.textContent=d.inserted_after_heading?"AI image generated and inserted after the selected heading.":"AI image generated and appended to the article.";
      articleImageStatus.className="cms-message success";
    }catch(err){articleImageStatus.textContent=err.message;articleImageStatus.className="cms-message error"}
    finally{btn.disabled=false}
  });
  ...
  form.addEventListener("submit",async e=>{
    ...
    saving=true;
    if(action==="autosave"){
      if($("#autosave-state"))$("#autosave-state").textContent="Saving…";
    }else{
      message.textContent=id?"Updating…":"Saving…";
    ...
    try{
      ...
      if(id && action!=="autosave"){
        await api("/rest/v1/rpc/snapshot_article_revision",{
          method:"POST",
          body:JSON.stringify({p_article_id:id,p_note:action==="publish"?"Before publish":action==="schedule"?"Before scheduling":"Before manual edit"})
        }).catch(()=>null);
      }
      ...
      if(action==="schedule"||action==="publish"){
        ...
        if(!gate?.can_publish){
          const failed=Object.entries(gate?.checks||{}).filter(([,ok])=>!ok).map(([key])=>key.replace(/^has_/,"").replaceAll("_"," "));
          throw new Error(`${action==="schedule"?"Schedule":"Publish"} gate failed${failed.length?": "+failed.join(", "):""}.`);
        }
      }
      ...
      if(action==="publish"){
        ...
        message.textContent="Article published.";
        ...
        if($("#autosave-state"))$("#autosave-state").textContent="Saved "+new Date().toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"});
      }else{
        message.textContent=originalStatus==="published"?"Published article changes saved.":originalStatus==="scheduled"?"Scheduled article changes saved.":"Draft saved.";
      }

      dirty=false;
      if(action!=="autosave")message.className="cms-message success";
      history.replaceState(null,"",`${ADMIN_BASE}/quick-entry?id=${id}`);
    }catch(err){
  ...
  setInterval(()=>{
    if(id&&dirty&&!saving&&document.visibilityState==="visible"){
      $("#autosave-submit")?.click();
    }
  },45000);
}
...
function md(markdown=""){
  ...
  let list=null,items=[];
  const flush=()=>{if(!list)return;out.push(`<${list}>${items.map(i=>`<li>${inline(i)}</li>`).join("")}</${list}>`);list=null;items=[]};
  const cells=line=>line.trim().replace(/^\||\|$/g,"").split("|").map(x=>x.trim());
  for(let i=0;i<lines.length;i++){
    const line=lines[i].trimEnd();if(!line.trim()){flush();continue}
    let m;
    if((m=line.match(/^:::cta\s+([^|]+)\|([^|]+)\|([^|]+)\|(.+)$/))){
      flush();const href=m[4].trim();
      if(/^\/|^https?:\/\//i.test(href))out.push(`<aside class="article-cta"><div><span class="eyebrow">NEXT STEP</span><h3>${inline(m[1].trim())}</h3><p>${inline(m[2].trim())}</p></div><a class="btn lime" href="${esc(href)}">${inline(m[3].trim())}</a></aside>`);
      continue;
    }
    if(line.trim().startsWith("|")&&i+1<lines.length&&/^\s*\|?\s*:?-{3,}/.test(lines[i+1])){
      flush();const heads=cells(line);i+=2;const rows=[];
      while(i<lines.length&&lines[i].trim().startsWith("|")){rows.push(cells(lines[i]));i++}i--;
      out.push(`<div class="article-table-wrap"><table class="article-table"><thead><tr>${heads.map(h=>`<th>${inline(h)}</th>`).join("")}</tr></thead><tbody>${rows.map(r=>`<tr>${heads.map((_,j)=>`<td>${inline(r[j]||"")}</td>`).join("")}</tr>`).join("")}</tbody></table></div>`);continue;
    }
    ...
    if((m=line.match(/^!\[([^\]]*)\]\(([^)\s]+)(?:\s+"([^"]*)")?\)$/))){flush();out.push(`<figure class="article-content-figure"><img src="${esc(m[2])}" alt="${esc(m[1]||"")}" loading="lazy">${m[3]?`<figcaption>${esc(m[3])}</figcaption>`:""}</figure>`);continue}
...
async function publicArticle(){
  ...
  const slug=params.get("slug")||(parts.length===2&&parts[0]==="blog"?parts[1]:null);
  const isArticleRoute=location.pathname.endsWith("/article.html")||(parts.length===2&&parts[0]==="blog");
...
async function publicHome(){
  ...
  if(pg)pg.innerHTML=articles.slice(0,4).map((a,i)=>`<a class="check" href="/blog/${esc(a.slug)}"><i>${i+1}</i><span>${esc(a.title)}</span></a>`).join("")||'<span class="subtle">Henüz yayınlanmış içerik yok.</span>';
}
async function publicSearch(){
  ...
  const run=async()=>{
    ...
    mount.innerHTML=`<div class="search-results-head"><div><div class="kicker" style="color:var(--pine)">Search results</div><h2>Results for “${esc(q)}”</h2></div><p class="subtle">${rows.length} result</p></div>`+(rows.map(a=>`<a class="result-card" href="/blog/${esc(a.slug)}">${a.featured_image_url?`<div class="result-media"><img src="${esc(a.featured_image_url)}" alt="${esc(a.featured_image_alt||"")}" loading="lazy"></div>`:`<div class="result-media is-empty"></div>`}<div class="result-copy"><div class="meta"><span class="badge">${esc(a.categories?.name||"Guide")}</span><span>${esc(a.content_type)}</span></div><h3>${esc(a.title)}</h3><p>${esc(a.excerpt||"Open this RV guide.")}</p></div></a>`).join("")||'<div class="no-results"><p>No matching published content.</p></div>');
  };
...
async function loadAdminView(){
  ...
  if(view==="dashboard"){
    const [rows,jobs,opps]=await Promise.all([
      api(`/rest/v1/articles?site_id=eq.${s.id}&select=id,title,slug,status,updated_at,published_at,scheduled_at,categories(name,slug)&order=updated_at.desc`),
      api(`/rest/v1/jobs?site_id=eq.${s.id}&select=id,status,job_type,created_at&order=created_at.desc&limit=20`).catch(()=>[]),
      api(`/rest/v1/seo_opportunities?site_id=eq.${s.id}&select=id,status,priority&order=detected_at.desc&limit=100`).catch(()=>[])
    ]);
    const counts=rows.reduce((a,x)=>{a.total++;a[x.status]=(a[x.status]||0)+1;return a},{total:0});
    const scheduled=rows.filter(x=>x.status==="scheduled").sort((a,b)=>new Date(a.scheduled_at)-new Date(b.scheduled_at));
    const review=(counts.editorial_qa||0)+(counts.technical_review||0)+(counts.ready||0);
    const failedJobs=jobs.filter(x=>["failed","error"].includes(String(x.status||"").toLowerCase())).length;
    const openOpps=opps.filter(x=>!["done","dismissed","closed"].includes(String(x.status||"").toLowerCase())).length;
    mount.innerHTML=`
      <section class="admin-hero-v2">
        <div><span class="eyebrow">Content operations</span><h2>Everything that needs attention, in one place.</h2><p>Manage drafts, scheduled publishing, performance and system health without jumping through unnecessary screens.</p></div>
        <div class="hero-actions"><a class="btn primary" href="${ADMIN_BASE}/articles">Open articles</a><a class="btn" href="${ADMIN_BASE}/calendar">View calendar</a></div>
      </section>
      <div class="kpis kpis-v2">
        <a class="kpi" href="${ADMIN_BASE}/articles"><span>Drafts</span><b>${counts.draft||0}</b><small>content waiting</small></a>
        <a class="kpi" href="${ADMIN_BASE}/articles"><span>Review queue</span><b>${review}</b><small>QA / ready</small></a>
        <a class="kpi" href="${ADMIN_BASE}/calendar"><span>Scheduled</span><b>${counts.scheduled||0}</b><small>upcoming posts</small></a>
        <a class="kpi" href="${ADMIN_BASE}/opportunities"><span>Opportunities</span><b>${openOpps}</b><small>growth ideas</small></a>
      </div>
      <div class="dashboard-grid-v2">
        <section class="table-card dashboard-card-v2">
          <div class="card-head-v2"><div><span class="eyebrow">Recent work</span><h3>Latest articles</h3></div><a href="${ADMIN_BASE}/articles">View all →</a></div>
          <table><thead><tr><th>Article</th><th>Status</th><th>Updated</th></tr></thead><tbody>
          ${rows.slice(0,7).map(a=>`<tr><td><a href="${ADMIN_BASE}/quick-entry?id=${a.id}"><b>${esc(a.title)}</b></a><br><span class="subtle">${esc(a.categories?.name||"Uncategorized")}</span></td><td><span class="status ${a.status==="draft"?"draft":a.status==="published"?"":"review"}">${statusLabel(a.status)}</span></td><td>${formatSiteDateTime(a.updated_at)}</td></tr>`).join("")||'<tr><td colspan="3">No content yet.</td></tr>'}
          </tbody></table>
        </section>
        <aside class="dashboard-stack-v2">
          <section class="admin-card">
            <div class="card-head-v2"><div><span class="eyebrow">Next up</span><h3>Publishing</h3></div><a href="${ADMIN_BASE}/calendar">Calendar →</a></div>
            ${scheduled.slice(0,4).map(a=>`<a class="dash-list-row" href="${ADMIN_BASE}/quick-entry?id=${a.id}"><div><b>${esc(a.title)}</b><small>${a.scheduled_at?formatSiteDateTime(a.scheduled_at):"Schedule missing"}</small></div><span>→</span></a>`).join("")||'<div class="empty-mini">Nothing scheduled yet.</div>'}
          </section>
          <section class="admin-card">
            <div class="card-head-v2"><div><span class="eyebrow">System</span><h3>Health snapshot</h3></div><a href="${ADMIN_BASE}/site-health">Details →</a></div>
            <div class="health-mini"><div><span>Published content</span><b>${counts.published||0}</b></div><div><span>Failed jobs</span><b class="${failedJobs?"danger-text":""}">${failedJobs}</b></div><div><span>Automation</span><b>Connected</b></div><div><span>Media</span><b>R2</b></div></div>
          </section>
        </aside>
      </div>
    `;
    ...
    const [articles,gsc,seoState]=await Promise.all([
      api(`/rest/v1/articles?site_id=eq.${s.id}&select=id,title,slug,status,seo_title,meta_description,canonical_path,featured_image_url,featured_image_alt`).catch(()=>[]),
      api(`/rest/v1/gsc_daily?site_id=eq.${s.id}&select=clicks,impressions,ctr,position&limit=5000`).catch(()=>[]),
      adminJson("/api/seo/settings").catch(()=>({settings:{},sitemap_url:"/sitemap.xml",robots_url:"/robots.txt"}))
    ]);
    const published=articles.filter(x=>x.status==="published");
    const issueRows=published.map(x=>{
      const issues=[];
      if(!x.seo_title)issues.push("SEO title");
      if(!x.meta_description)issues.push("Meta description");
      if(!x.canonical_path)issues.push("Canonical");
      if(x.canonical_path&&x.slug&&!String(x.canonical_path).includes(`/blog/${x.slug}`))issues.push("Canonical path");
      if(!x.featured_image_url)issues.push("Featured image");
      if(x.featured_image_url&&!x.featured_image_alt)issues.push("Image alt");
      return {...x,issues};
    }).filter(x=>x.issues.length);
    const clicks=Math.round(gsc.reduce((a,x)=>a+Number(x.clicks||0),0));
    const impr=Math.round(gsc.reduce((a,x)=>a+Number(x.impressions||0),0));
    const cfg=seoState.settings||{};
    ...
    setChecked("#seo-sitemap-static",cfg.sitemap_include_static);
    setChecked("#seo-robots-enabled",cfg.robots_enabled);
    setChecked("#seo-oai-enabled",cfg.allow_oai_searchbot);
    if($("#seo-extra-robots"))$("#seo-extra-robots").value=cfg.extra_robots||"";

    mount.innerHTML=`
      <div class="kpis">
        <div class="kpi"><span>YAYINDA</span><b>${published.length}</b></div>
        <div class="kpi"><span>SEO UYARISI</span><b>${issueRows.length}</b></div>
        <div class="kpi"><span>GSC CLICKS</span><b>${clicks}</b></div>
        <div class="kpi"><span>GSC IMPRESSIONS</span><b>${impr}</b></div>
      </div>
      <div class="seo-admin-grid">
        <section class="admin-card seo-admin-settings">
          ...
          <form id="seo-core-form">
            ...
            <label class="field"><span>Additional robots.txt rules</span><textarea id="seo-extra-robots" rows="7" placeholder="Example:&#10;Disallow: /temporary-area/">${esc(cfg.extra_robots||"")}</textarea><small>These lines are appended after the protected admin/API rules. Sitemap stays automatic.</small></label>
            <div class="seo-admin-actions">
              <button type="submit" class="btn primary" id="seo-save-settings">Save SEO settings</button>
              <a class="btn" href="${esc(seoState.sitemap_url||"/sitemap.xml")}" target="_blank" rel="noopener">Open sitemap</a>
              <a class="btn" href="${esc(seoState.robots_url||"/robots.txt")}" target="_blank" rel="noopener">Open robots.txt</a>
            </div>
            <div id="seo-settings-message" class="cms-message" aria-live="polite"></div>
          </form>
        </section>
        <section class="admin-card seo-admin-health">
          <div class="seo-admin-head"><div><span class="subtle">Automatic checks</span><h3>Technical SEO health</h3></div></div>
          <div class="seo-health-list">
            <div><span>Canonical blog pattern</span><b>/blog/article-slug</b></div>
            <div><span>Blog index</span><b>/blog</b></div>
            <div><span>Dynamic sitemap</span><b>${cfg.sitemap_enabled===false?"Disabled":"Enabled"}</b></div>
            <div><span>robots.txt</span><b>${cfg.robots_enabled===false?"Disabled":"Enabled"}</b></div>
            <div><span>Published URLs in DB</span><b>${published.length}</b></div>
          </div>
          <p class="subtle" style="margin-top:16px">Search pages and 404s remain noindex. Public articles, categories, the homepage and the blog index are indexable.</p>
        </section>
      </div>
      ${issueRows.length?`
        <div class="table-card seo-issues-table">
          <div class="seo-admin-head"><div><span class="subtle">Content-level checks</span><h3>SEO warnings</h3></div><span>${issueRows.length} article${issueRows.length===1?"":"s"}</span></div>
          <table><thead><tr><th>Content</th><th>Warnings</th><th></th></tr></thead><tbody>
            ${issueRows.slice(0,30).map(x=>`<tr><td><b>${esc(x.title)}</b><br><span class="subtle">/blog/${esc(x.slug)}</span></td><td>${x.issues.map(y=>`<span class="seo-issue-chip">${esc(y)}</span>`).join(" ")}</td><td><a href="${ADMIN_BASE}/quick-entry?id=${x.id}">Edit →</a></td></tr>`).join("")}
          </tbody></table>
        </div>`:`
        <div class="admin-card seo-clean-card"><b>No critical content-level SEO warnings found.</b><p class="subtle">Published items currently have the core metadata expected by the CMS checks.</p></div>`}
      ${gsc.length?"":emptyCard("GSC verisi henüz senkronlanmamış.")}
    `;

    $("#seo-core-form")?.addEventListener("submit",async ev=>{
      ev.preventDefault();
      const btn=$("#seo-save-settings"),msg=$("#seo-settings-message");
      btn.disabled=true;msg.textContent="Saving…";msg.className="cms-message";
      try{
        const payload={
          site_name:$("#seo-site-name").value.trim(),
          default_meta_description:$("#seo-default-description").value.trim(),
          sitemap_enabled:$("#seo-sitemap-enabled").checked,
          sitemap_include_categories:$("#seo-sitemap-categories").checked,
          sitemap_include_static:$("#seo-sitemap-static").checked,
          robots_enabled:$("#seo-robots-enabled").checked,
          allow_oai_searchbot:$("#seo-oai-enabled").checked,
          extra_robots:$("#seo-extra-robots").value
        };
        await adminJson("/api/seo/settings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});
        msg.textContent="SEO settings saved. robots.txt and sitemap now use the new configuration.";
...
function initBlogEnhancements(){
  const toc=document.querySelector(".rvf-article-toc");
  if(!toc)return;
  ...
  links.forEach(link=>{
    link.addEventListener("click",e=>{
      ...
      const offset=(header?.getBoundingClientRect().height||76)+22;
  ...
  if("IntersectionObserver" in window && targets.length){
    const byId=new Map(targets.map(x=>[x.target.id,x.link]));
    ...
    targets.forEach(x=>observer.observe(x.target));
  }

      document.querySelectorAll(".rvf-ai-logo img").forEach(img=>{
  ...
  document.querySelectorAll("[data-ai-open]").forEach(btn=>{
    btn.addEventListener("click",async()=>{
      ...
      const status=document.querySelector(".rvf-ai-copy-status");
      let copied=false;
      ...
      if(provider==="chatgpt")target="https://chatgpt.com/?prompt="+encoded;
      else if(provider==="perplexity")target="https://www.perplexity.ai/search?q="+encoded;
      else if(provider==="grok")target="https://grok.com/?q="+encoded;
      else if(provider==="claude")target="https://claude.ai/new?q="+encoded;
...
loadArticles().catch(console.error);initEditor().catch(console.error);loadAdminView().catch(console.error);initMediaLibrary().catch(console.error);gscSetup().catch(console.error);initBlogEnhancements();publicHome().catch(console.error);publicSearch().catch(console.error);publicArticle().catch(console.error);publicCategory().catch(console.error);
